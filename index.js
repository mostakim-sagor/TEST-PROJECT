...
// storedHash — server শুরু হওয়ার আগেই load করা হচ্ছে
let storedHash = null;
try {
...
const server = http.createServer(async (req, res) => {
	...
	if (req.method === "POST" && url === "/auth/login") {
		try {
			...
			if (clientHash !== storedHash) return jsonErr(res, 401, "Wrong password");
	...
	if (req.method === "GET" && url === "/status") {
		try {
			...
			jsonOk(res, {
				...
				ram:  { used: (usedMem / 1e9).toFixed(2), total: (totalMem / 1e9).toFixed(2), percent: ((usedMem / totalMem) * 100).toFixed(1) },
				heap: { used: (mem.heapUsed / 1e6).toFixed(1), total: (mem.heapTotal / 1e6).toFixed(1), percent: ((mem.heapUsed / mem.heapTotal) * 100).toFixed(1) },
				cpu_percent: null
			});
...
server.listen(DASHBOARD_PORT, "0.0.0.0", () => {
	log.success("DASHBOARD", `Running at http://0.0.0.0:${DASHBOARD_PORT}`);
});

server.on("error", (err) => {
	log.error("DASHBOARD", `Server error: ${err.message}`);
	if (err.code === "EADDRINUSE") log.error("DASHBOARD", `Port ${DASHBOARD_PORT} is already in use!`);
});
...
function startProject(reason) {
	...
	setTimeout(() => {
		...
		delete childEnv.PORT; // ✅ bot কে PORT দেওয়া হচ্ছে না — port conflict ঠেকাতে
