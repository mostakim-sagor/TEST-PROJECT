...
// Client sends sha256(plain), server stores sha256(sha256(plain))
function loadPasswordHash() {
        ...
        if (fs.existsSync(DASH_CONFIG_FILE)) {
                try {
                        ...
                        if (plain && plain.length >= 4) {
                                // Hash and persist securely
                                const clientHash = sha256(plain);
                                const serverHash = sha256(clientHash);
                                fs.writeFileSync(PASSWORD_FILE, serverHash, "utf-8");

                                // Wipe plain password from file immediately
                                cfg.password = "";
                                cfg._status  = "Password saved securely. Plain text cleared.";
                                fs.writeFileSync(DASH_CONFIG_FILE, JSON.stringify(cfg, null, 2), "utf-8");

                                log.success("DASHBOARD", `Password set from dashboard.config.json and secured.`);
                                return serverHash;
                        }
                } catch (e) {
                        log.warn("DASHBOARD", `dashboard.config.json parse error: ${e.message}`);
                }
        }

        // 2️⃣  Environment variable (Replit Secret / Railway / Render)
        if (process.env.DASHBOARD_PASSWORD) {
                const clientHash = sha256(process.env.DASHBOARD_PASSWORD);
        ...
        if (fs.existsSync(PASSWORD_FILE)) {
                ...
                if (saved.length === 64) return saved; // valid sha256 hex
        }

        // 4️⃣  First run: auto-generate
        const generated  = crypto.randomBytes(6).toString("hex"); // 12-char
        const clientHash = sha256(generated);
        const serverHash = sha256(clientHash);
        fs.writeFileSync(PASSWORD_FILE, serverHash, "utf-8");
        ...
        log.success("DASHBOARD", `  Put your password in "password" and restart.`);
        log.success("DASHBOARD", `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
        return serverHash;
}

// storedHash = sha256(sha256(plainPassword))
let storedHash = null;
try { storedHash = loadPasswordHash(); } catch (e) { log.error("DASHBOARD", `Password init error: ${e.message}`); }
...
function startKeepAlive(intervalSec = 180) {
        ...
        if (!url) { log("Keep-alive: No public URL detected. Skipping self-ping.", "WARN"); return; }
        const pingUrl = url.replace(/\/$/, "") + "/ping";
        log(`Keep-alive: Pinging ${pingUrl} every ${intervalSec}s`, "INFO");
        setInterval(() => {
                try {
...
const server = http.createServer(async (req, res) => {
        ...
        // ── PUBLIC: Ping (for uptime monitors — no auth needed) ──
        if (req.method === "GET" && url === "/ping") {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ ok: true, uptime: Math.floor(process.uptime()), time: Date.now() }));
                return;
        }
        ...
        if (req.method === "GET" && url === "/health") {
                ...
                res.end(JSON.stringify({ ok: alive, uptime: Math.floor(process.uptime()) }));
                return;
        }

        // ── AUTH: Login ──
        // Client sends { clientHash: sha256(plainPassword) }
        // Server checks sha256(clientHash) === storedHash
        if (req.method === "POST" && url === "/auth/login") {
                try {
                        ...
                        const serverSide = sha256(clientHash);
                        if (!storedHash || serverSide !== storedHash)
        ...
        if (req.method === "GET" && url === "/auth/verify") {
                const token = getToken(req);
                return jsonOk(res, { ok: isValidSession(token) });
        }
        ...
        if (req.method === "POST" && url === "/auth/change-password") {
                try {
                        ...
                        const newServerHash = sha256(newClientHash);
                        fs.writeFileSync(PASSWORD_FILE, newServerHash, "utf-8");
                        storedHash = newServerHash;
...
server.listen(DASHBOARD_PORT, "0.0.0.0", () => {
        log(`Dashboard running at http://localhost:${DASHBOARD_PORT}`, "INFO");
});
...
function startProject(reason) {
        ...
        if (backoff > 0) log(`Bot restarting in ${backoff / 1000}s (attempt #${restartCount})...`, "WARN");
        else if (reason) log(`Bot starting (${reason})...`, "INFO");
        setTimeout(() => {
                ...
                botProcess.on("close", (code, signal) => {
                        ...
                        if (code === 2)       { log("Bot requested restart (code 2).", "INFO"); startProject("code-2"); }
                        else if (code === 0)  { log("Bot exited cleanly.", "INFO"); }
                        else                  { log(`Bot crashed (code=${code}). Restarting...`, "WARN"); startProject("crash"); }
                });
